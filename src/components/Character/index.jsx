...
const Character = ({ infos, onDetails }) => {
	...
	return (
		<FlexBox className="character-flexbox" hAlign={Alignment.spaceBetween}>
			...
			<FlexBox className="character-infos-flexbox" vAlign={Alignment.spaceBetween}>
				<Link className="link" onClick={() => onDetails(infos.id)} to={characterDetailsUrl}><h1 className="character-name">{infos.name}</h1></Link>
				<div className="character-description">{infos.description}</div>
				<Link className="link" onClick={() => onDetails(infos.id)} to={characterDetailsUrl}><Button className="see-details-button">See details</Button></Link>
			</FlexBox>
		</FlexBox>
	);
}
